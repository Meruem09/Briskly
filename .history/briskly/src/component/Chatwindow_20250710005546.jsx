...
useEffect(() => {
  const fetchChats = async () => {
    ...
    // Load last used chatId from localStorage
    const lastChatId = localStorage.getItem('chatId');
    const validChatId = res.data.find(chat => chat.id === Number(lastChatId));

    if (validChatId) {
      setSelectedChatId(validChatId.id);
      setMessages(validChatId.messages);
    } else if (res.data.length > 0) {
      // Pick latest chat if no stored chatId
